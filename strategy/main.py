...
def _normalize_ohlc_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in list(df.columns):
        low = col.lower()
        if low.startswith("open") and "open" not in df.columns:
            ren[col] = "open"
        if low.startswith("high") and "high" not in df.columns:
            ren[col] = "high"
        if low.startswith("low") and "low" not in df.columns:
            ren[col] = "low"
        if low.startswith("close") and "close" not in df.columns:
            ren[col] = "close"
    if ren:
...
def load_bars(symbol: str, start: str, end: str) -> pd.DataFrame:
    ...
    try:
        if symbol.upper().endswith((".SZ", ".SS")) or symbol.isdigit():
            ...
            out = _normalize_ohlc_df(
                df.set_index("date")[["open", "high", "low", "close"]]
            )
            if len(out) > 0:
                return out
...
from strategy_v12 import backtest as run_v12
...
def save_strategy_xlsx(
    ...
    if engine is None:
        raise RuntimeError(
            "未检测到 openpyxl/xlsxwriter，请先安装：pip install openpyxl"
        )
    safe_label = label.replace("/", "_").replace(" ", "")
    xlsx_path = os.path.join(
        OUTPUT_DIR, f"report_{symbol}_{start}_{end}_{safe_label}.xlsx"
    )
    with pd.ExcelWriter(
        xlsx_path, engine=engine, datetime_format="yyyy-mm-dd", date_format="yyyy-mm-dd"
    ) as w:
...
def save_comparison_xlsx(
    symbol: str, start: str, end: str, curves: list[pd.DataFrame], labels: list[str]
):
    ensure_outdir(OUTPUT_DIR)
    engine = get_excel_engine()
    if engine is None:
    ...
    for c, lb in zip(curves, labels):
        df = df.merge(
            c[["date", "equity"]].rename(columns={"equity": lb}), on="date", how="left"
        )
...
def main():
    ...
    print("运行 Strategy v12 …")
    curve3, trades3, label3 = run_v12(bars, INITIAL_CASH, **kwargs)
    ...
    # 对齐日期后导出对比（XLSX + PNG）
    all_dates = pd.Series(
        sorted(set(curve1["date"]) | set(curve2["date"]) | set(curve3["date"]))
    )
    curve1a = pd.merge(
        all_dates.to_frame(name="date"), curve1, on="date", how="left"
    ).ffill()
    curve2a = pd.merge(
        all_dates.to_frame(name="date"), curve2, on="date", how="left"
    ).ffill()
    curve3a = pd.merge(
        all_dates.to_frame(name="date"), curve3, on="date", how="left"
    ).ffill()
    save_comparison_xlsx(
        SYMBOL, START, END, [curve1a, curve2a, curve3a], [label1, label2, label3]
    )
    save_comparison_png(
        SYMBOL,
        START,
        END,
        [curve1a, curve2a, curve3a],
        [label1, label2, label3],
        title=f"{SYMBOL} Backtest (V1/V2/V12)",
    )
